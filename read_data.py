...
for file_num in range(1, 13):
    if file_num == 7: # Reflectance recorded with mobile camera at Fs = 1/30
        Fs = 1/30
    if file_num == 11:
        Fs = 1/40
...
for num_of_meas in range(0, (len(pulse) // 3)):
    expected = [75, 71, 65, 70, 72, 69, 62, 67, 69, 66, 72, 82]
...
for i in range(0, len(pulse)):
    table.append([meas[i], channels[i], "{0:.1f} BPM".format(pulse[i]),
        "{0:.1f} BPM".format(expected[i]), "{0:.1f} BPM".format(deviance[i])])

with open("resultater/results.txt", "w") as results: # write results to file
    results.write(tabulate(table,
        headers=["Measurement #", "Channels", "Heartbeat", "Expected BPM", "Deviance"],
        tablefmt="fancy_grid" ))
